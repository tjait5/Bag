...
enum GameState {
    ...
    case roundOver
    case gameOver
    case roundOverExceed    //Used with 21 exact rules, if exceed, then present alert that you exceeded score threshold
}
...
class GameScorer: NSObject, NSCoding {
    
    struct Keys{
        static let game = "game"
        static let isWinByExact = "isWinByExact"
    }
    
    var game: Game = Game()
    var isWinByExact:  Bool = UserDefaults.standard.bool(forKey: "ruleState")
    ...
    init(game: Game = Game()) {
        self.game = game
        self.isWinByExact = UserDefaults.standard.bool(forKey: "ruleState")
    }
    
    //MARK::NSCoding
    func encode(with aCoder: NSCoder) {
        aCoder.encode(game, forKey: Keys.game)
        aCoder.encode(isWinByExact, forKey: Keys.isWinByExact)
    }
    
    required init?(coder aDecoder: NSCoder) {
        if let gameObject = aDecoder.decodeObject(forKey: Keys.game) as? Game{
            self.game = gameObject
        }
        if let isWinByExactBool = aDecoder.decodeObject(forKey: Keys.isWinByExact) as? Bool{
            self.isWinByExact = isWinByExactBool
        }
        super.init()
    }
    
    //MARK::Public
    func setRuleState(for state: Bool){
        isWinByExact = state
    }
    ...
    /// Returns score for all rounds played so far
    func scoreForAllRounds(for team:  Team) -> Int{
        var scoreForAllRounds = 0
        for round in self.game.rounds{
            
            let redScore = self.scoreInRound(round, forTeam: .red)
            let blueScore = self.scoreInRound(round, forTeam: .blue)
            
            if(team == .red){
                if(redScore - blueScore > 0){
                    scoreForAllRounds += redScore - blueScore
                }
            }
            else{
                if(blueScore - redScore > 0){
                    scoreForAllRounds += blueScore - redScore
                }
            }
        }
        return scoreForAllRounds
    }
    ...
    func scoreInRound(_ round: Round, forTeam team: Team) -> Int {
        var score = 0
        let throwsInRound = self.throwsInRound(round, forTeam: team)
        for indexThrow in throwsInRound{
            if let value = indexThrow.value?.rawValue{
                score += value
            }
        }
        return score
    ...
    func isGameInProgress() -> Bool{
        if(self.game.currentRound.blueThrows.count > 0 || self.game.currentRound.redThrows.count > 0){
            return true
        }
        else{
            return false
    ...
    func startNewRound() {
        ...
        if case .over(let winner, _) = self.result(forRound: lastRound) {
            ...
            self.game.setHasSetScoreForRound(lock: false)
        }
    }
    
    func gameWinner() -> Team? {
    ...
    private func isGameOver() -> Bool {
        ...
        //Keep track of total score on game
        if(self.isRoundOver(self.game.currentRound)){
            self.game.setScore(redScore: self.scoreInRound(self.game.currentRound, forTeam: .red), blueScore: self.scoreInRound(self.game.currentRound, forTeam: .blue))
        }
        
        let redScore = self.game.redScore
        let blueScore = self.game.blueScore
        
        if(self.isWinByExact){
            return (redScore == 21 || blueScore == 21)
        }
        else{
    ...
    private func currentGameState() -> GameState {
        if self.isGameOver() {
            ...
            if(self.isWinByExact && (self.game.blueScore > 21 || self.game.redScore > 21)){
                if(self.game.redScore > 21){
                    self.game.setRedScore(score: 11)
                }
                if(self.game.blueScore > 21){
                    self.game.setBlueScore(score: 11)
                }
                return .roundOverExceed
            }
            else{
                return .roundOver
            }
